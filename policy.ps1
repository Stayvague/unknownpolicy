...
$Flagged = @('software.exe','tiworker.exe','loader.exe',
    ...
    foreach ($f in $Flagged){
        if ($leaf -eq $f){
            if ($f -eq 'tiworker.exe'){
                ...
                if ($sig.Status -ne 'Valid' -or $sig.SignerCertificate.Subject -notmatch 'Microsoft'){ return $true }
                return $false
            }
            return $true
        }
...
if (Serial-State $biosSerial){ Line ("   BIOS serial  : present - normal") Green } else { Line ("   BIOS serial  : blank / placeholder") DarkYellow }
$boardSerial = if ($board){ $board.SerialNumber } else { $null }
if (Serial-State $boardSerial){ Line ("   Board serial : present - normal") Green } else { Line ("   Board serial : blank / placeholder") DarkYellow }
$di=0
foreach ($d in $disks){
    $di++
    if (Serial-State $d.SerialNumber){ Line ("   Disk $di serial: present - normal") Green } else { Line ("   Disk $di serial: blank / placeholder") DarkYellow }
}
...
if (Test-Path $am){
    ...
    if ($kb -lt 256){ Note $WARN "Amcache: only ${kb}KB - unusually small." } else { Note $PASS "Amcache: present (${kb}KB)." }
...
if (Test-Path $srum){
    ...
    if ($mb -lt 1){ Note $WARN "SRUM: only ${mb}MB - likely reset." } else { Note $PASS "SRUM: present (${mb}MB)." }
...
if (Test-Path $hist){
    ...
    if ($h.Count -eq 0){ Note $WARN "PS history: empty - cleared." } else { Note $PASS "PS history: $($h.Count) lines present." }
...
foreach ($proc in (Get-Process -ErrorAction SilentlyContinue)){
    ...
    if (Test-Flagged $nm){ $pHit++
        Note $FAIL "Process: flagged LIVE -> $($proc.ProcessName) (PID $($proc.Id)) $(if($path){"at $path"}else{'[path hidden]'})" }
